...
async function loginToDoorDash(page) {
  try {
    ...
    console.log('Login page inputs:', JSON.stringify(pageContent.inputs));
    console.log('Login page text:', pageContent.text);

    // Try all possible email input selectors
    ...
    for (const sel of emailSelectors) {
      ...
      if (emailInput) { console.log('Found email input with selector: ' + sel); break; }
    }

    if (!emailInput) throw new Error('Email input not found on page');
    await emailInput.click();
    await emailInput.type(process.env.DOORDASH_EMAIL, { delay: 80 });
    await new Promise(r => setTimeout(r, 1000));

    // Click continue button
    const btnSelectors = [
      'button[type="submit"]',
      'button[data-anchor-id*="login"]',
      'button[data-anchor-id*="continue"]',
      'button[data-anchor-id*="submit"]',
      'button',
    ];
    let btn = null;
    for (const sel of btnSelectors) {
      ...
      for (const b of btns) {
        ...
        if (text && (text.toLowerCase().includes('continue') || text.toLowerCase().includes('next') || text.toLowerCase().includes('log in') || text.toLowerCase().includes('sign in'))) {
          btn = b;
          break;
        }
      }
    ...
    if (passwordInput) {
      ...
      await passwordInput.type(process.env.DOORDASH_PASSWORD, { delay: 80 });
      await page.keyboard.press('Enter');
      await new Promise(r => setTimeout(r, 3000));
    }
