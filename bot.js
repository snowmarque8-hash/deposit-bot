...
// Use Railway volume at /data if it exists, otherwise local folder
const DATA_DIR = fs.existsSync('/data') ? '/data' : '.';
const DB_FILE = DATA_DIR + '/balances.json';
const LOG_FILE = DATA_DIR + '/transactions.json';
...
// Points - stored with monthly period tracking
function loadPoints() {
  if (!fs.existsSync(POINTS_FILE)) {
    const fresh = { _meta: { periodStart: new Date().toISOString() }, users: {} };
    fs.writeFileSync(POINTS_FILE, JSON.stringify(fresh, null, 2));
    return fresh;
  }
  let data;
  try { data = JSON.parse(fs.readFileSync(POINTS_FILE, 'utf8')); }
  catch (e) { data = {}; }

  // Already in correct format
  if (data._meta && data.users) return data;

  // Migrate old flat format - move real user entries into users object
  const users = {};
  for (const key of Object.keys(data)) {
    if (key === '_meta' || key === 'users') continue;
    if (data[key] && typeof data[key] === 'object' && 'points' in data[key]) {
      users[key] = data[key];
    }
  }
  const migrated = { _meta: { periodStart: new Date().toISOString() }, users };
  savePoints(migrated);
  return migrated;
}
...
function addPoint(userId, username) {
  const p = loadPoints();
  if (!p.users[userId]) p.users[userId] = { username, points: 0 };
  p.users[userId].points += 1;
  p.users[userId].username = username;
  savePoints(p);
  return p.users[userId].points;
}
function resetAllPoints() {
  const p = loadPoints();
  p.users = {};
  p._meta.periodStart = new Date().toISOString();
  savePoints(p);
}
function checkMonthlyReset() {
  const p = loadPoints();
  const start = new Date(p._meta.periodStart);
  const daysPassed = (Date.now() - start.getTime()) / (1000 * 60 * 60 * 24);
  if (daysPassed >= 30) {
    console.log('30 days passed - auto-resetting points');
    resetAllPoints();
    return true;
  }
  return false;
}
function getPointsPeriodInfo() {
  const p = loadPoints();
  const start = new Date(p._meta.periodStart);
  const daysPassed = (Date.now() - start.getTime()) / (1000 * 60 * 60 * 24);
  const daysLeft = Math.max(0, Math.ceil(30 - daysPassed));
  return { start, daysLeft };
}
...
client.on('messageCreate', async function(message) {
  ...
  if (drdMatch) {
    ...
    // Check if it expanded to a cart/group order
    if (fullUrl.includes('group-order') || fullUrl.includes('cart') || fullUrl.includes('drd.sh')) {

      // Extract cart UUID from URL params or path
      let cartId = null;
      try {
        const urlObj = new URL(fullUrl);
        cartId = urlObj.searchParams.get('orderCartId')
          || urlObj.searchParams.get('next')?.match(/[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}/i)?.[0]
          || fullUrl.match(/[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}/i)?.[0];
      } catch (e) {}

      const token = process.env.DOORDASH_TOKEN;

      async function fetchCartOrder(id) {
        if (!id) return null;
        const endpoints = [
          'https://www.doordash.com/api/v2/cart/' + id + '/',
          'https://www.doordash.com/api/v2/group_orders/' + id + '/',
          'https://www.doordash.com/api/v1/cart/' + id + '/',
        ];
        for (const ep of endpoints) {
          try {
            const r = await fetch(ep, {
              headers: {
                'Authorization': 'Bearer ' + (token ? token.split(':')[0] : ''),
                'Cookie': 'ddweb_token=' + token,
                'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) AppleWebKit/605.1.15',
                'Accept': 'application/json',
      ...
      // First try Puppeteer scraper for full details
      const scraped = await scrapeCartLink(fullUrl);
      ...
      // Parse creator from URL as fallback
      try {
        const urlObj = new URL(fullUrl);
        creator = urlObj.searchParams.get('orderCartCreator') || urlObj.searchParams.get('creatorNameAbbreviated') || 'Unknown';
      } catch (e) {}

      if (scraped.success && scraped.data) {
        const d = scraped.data;
        store = d.restaurant || store;
        creator = d.creator || creator;
        items = d.items || [];
        subtotal = d.subtotal || 0;
        // Store item count even if items array is empty
        if (d.itemCount !== undefined) {
          itemCount = d.itemCount;
        }
      } else {
        // Fallback to API
        const cartData = cartId ? await fetchCartOrder(cartId) : null;
        if (cartData) {
          store = cartData.store_name || (cartData.store && cartData.store.name) || store;
          items = cartData.items || (cartData.cart && cartData.cart.items) || [];
          subtotal = (cartData.subtotal || (cartData.cart && cartData.cart.subtotal) || 0) / 100;
        }
      }
      ...
      let itemsText = items.length
        ? items.map(function(item) {
            ...
            const promo = total > 0 ? applyPromo(total, 50) : null;
            return qty + 'x ' + name + (promo ? ' | $' + total.toFixed(2) + ' -> $' + promo.final : '');
          }).join('\n')
        : (itemCount > 0 ? itemCount + ' item(s) in cart - Sign into DoorDash to see details' : 'No items yet');

      const embed = new EmbedBuilder()
        .setColor(0xFF3008)
        .setTitle('DoorDash Shared Cart')
        .setURL(fullUrl)
        .addFields(
      ...
      if (subtotal > 0) {
        embed.addFields(
          ...
          { name: 'Summary', value: 'Original: $' + subtotal.toFixed(2) + '\nDiscount: -$' + promoSubtotal.discount + '\nYou pay: $' + promoSubtotal.final },
        );
      }

      embed.addFields({ name: 'Items', value: itemsText.slice(0, 1024) });
      if (pointsText) embed.addFields({ name: 'Points', value: pointsText });
      embed.setFooter({ text: 'Live Tracker | Updates every 30s' }).setTimestamp();
      ...
      const drdInterval = setInterval(async function() {
        try {
          const updated = cartId ? await fetchCartOrder(cartId) : null;
          const updatedItems = (updated && (updated.items || (updated.cart && updated.cart.items))) || [];
          const updatedSubtotal = updated ? (updated.subtotal || (updated.cart && updated.cart.subtotal) || 0) / 100 : 0;
          const updatedPromo = applyPromo(updatedSubtotal, 50);
          const updatedStore = (updated && (updated.store_name || (updated.store && updated.store.name))) || store;
          ...
          const updatedEmbed = new EmbedBuilder()
            .setColor(0xFF3008).setTitle('DoorDash Shared Cart').setURL(fullUrl)
            .addFields(
              { name: 'Created by', value: creator, inline: true },
          ...
          if (updatedSubtotal > 0) {
            updatedEmbed.addFields(
              ...
              { name: 'Summary', value: 'Original: $' + updatedSubtotal.toFixed(2) + '\nDiscount: -$' + updatedPromo.discount + '\nYou pay: $' + updatedPromo.final },
            );
          }

          updatedEmbed.addFields({ name: 'Items', value: updatedItemsText.slice(0, 1024) });
          updatedEmbed.setFooter({ text: 'Live Tracker | Updated: ' + new Date().toLocaleTimeString() }).setTimestamp();
          await sentMsg.edit({ embeds: [updatedEmbed] });
        } catch (e) { console.error('drd tracker error:', e.message); }
      }, 30000);
  ...
  if (groupMatch) {
    ...
    embed.setFooter({ text: 'Based on 50% promo | Updates every 30s' }).setTimestamp();
  ...
  // doomdash.online tracker link
  const doomMatch = message.content.match(DOOMDASH_REGEX);
  if (doomMatch) {
    const url = doomMatch[0];
    const thinking = await message.reply('Reading your DoomDash order...');
    ...
    function buildDoomEmbed(data, lastUpdated) {
      const embed = new EmbedBuilder().setColor(0xFF3008).setTitle('DoomDash Order Tracker').setURL(url);
      if (data) {
        embed.addFields(
          { name: 'Status', value: data.status || 'Unknown', inline: true },
          { name: 'Restaurant', value: data.restaurant || 'Unknown', inline: true },
        ...
        if (data.eta) embed.addFields({ name: 'ETA', value: data.eta, inline: true });
        if (data.customer) embed.addFields({ name: 'Order For', value: data.customer, inline: true });
        if (data.dasher) embed.addFields({ name: 'Dasher', value: data.dasher.rating + ' stars, ' + data.dasher.deliveries + ' deliveries', inline: true });
        if (data.items && data.items.length) {
          const itemsText = data.items.map(function(i) { return i.qty + 'x ' + i.name; }).join('\n');
          embed.addFields({ name: 'Items', value: itemsText.slice(0, 1024) });
        }
    ...
    const initial = await scrapeDoomDash(url);
    const sentMsg = await thinking.edit({ content: '', embeds: [buildDoomEmbed(initial, null)] });

    const doomInterval = setInterval(async function() {
      try {
        const updated = await scrapeDoomDash(url);
        await sentMsg.edit({ embeds: [buildDoomEmbed(updated, new Date().toLocaleTimeString())] });
        if (updated && updated.status === 'Delivered') clearInterval(doomInterval);
      } catch (e) { console.error('DoomDash tracker error:', e.message); }
    }, 30000);

    setTimeout(function() { clearInterval(doomInterval); }, 3 * 60 * 60 * 1000);
    return;
  }

  // ddtrack.live tracker link
  const ddtrackMatch = message.content.match(DDTRACK_REGEX);
  if (ddtrackMatch) {
    const url = ddtrackMatch[0];
    ...
    function buildDDTrackEmbed(data, lastUpdated) {
      const embed = new EmbedBuilder().setColor(0xFF3008).setTitle('DoorDash Order Tracker').setURL(url);
      if (data) {
        embed.addFields(
          { name: 'Status', value: data.status || 'Unknown', inline: true },
          { name: 'Restaurant', value: data.restaurant || 'Unknown', inline: true },
        );
        if (data.dasher) embed.addFields({ name: 'Dasher', value: data.dasher, inline: true });
        if (data.deliveringTo) embed.addFields({ name: 'Delivering To', value: data.deliveringTo, inline: true });
        if (data.items && data.items.length) {
          const itemsText = data.items.map(function(i) { return i.qty + 'x ' + i.name; }).join('\n');
          embed.addFields({ name: 'Items', value: itemsText.slice(0, 1024) });
        }
    ...
    const initial = await scrapeDDTrack(url);
    const sentMsg = await thinking.edit({ content: '', embeds: [buildDDTrackEmbed(initial, null)] });

    const ddtrackInterval = setInterval(async function() {
      try {
        const updated = await scrapeDDTrack(url);
        await sentMsg.edit({ embeds: [buildDDTrackEmbed(updated, new Date().toLocaleTimeString())] });
        if (updated && (updated.status === 'Delivered' || updated.status === 'Cancelled')) {
          clearInterval(ddtrackInterval);
        }
      } catch (e) { console.error('DDTrack tracker error:', e.message); }
    }, 30000);

    setTimeout(function() { clearInterval(ddtrackInterval); }, 3 * 60 * 60 * 1000);
    return;
  }
...
client.on('interactionCreate', async function(interaction) {
  ...
  if (cmd === 'points') {
    checkMonthlyReset();
    const p = loadPoints();
    const sorted = Object.entries(p.users).sort(function(a, b) { return b[1].points - a[1].points; }).slice(0, 10);
    if (!sorted.length) return interaction.reply({ content: 'No points yet! Chefs earn points by sharing DoorDash links.', ephemeral: true });

    const medals = ['🥇', '🥈', '🥉'];
    const totalPoints = Object.values(p.users).reduce(function(sum, u) { return sum + u.points; }, 0);
    const info = getPointsPeriodInfo();

    const lines = sorted.map(function(entry, i) {
      const rank = medals[i] || ('**#' + (i + 1) + '**');
      const pts = entry[1].points;
      const bar = '▰'.repeat(Math.min(10, pts)) + '▱'.repeat(Math.max(0, 10 - Math.min(10, pts)));
      return rank + ' <@' + entry[0] + '>\n` ' + bar + ' ` **' + pts + '** pt' + (pts !== 1 ? 's' : '');
    });
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: 'Active Chefs', value: String(sorted.length), inline: true },
        { name: 'Resets In', value: info.daysLeft + ' day' + (info.daysLeft !== 1 ? 's' : ''), inline: true },
      )
      .setFooter({ text: 'Earn points by sharing DoorDash order links!' })
      .setTimestamp();
  ...
  if (cmd === 'mypoints') {
    checkMonthlyReset();
    const p = loadPoints();
    const me = p.users[interaction.user.id];
    const pts = me ? me.points : 0;

    // Calculate rank
    const sorted = Object.entries(p.users).sort(function(a, b) { return b[1].points - a[1].points; });
    const rank = sorted.findIndex(function(e) { return e[0] === interaction.user.id; }) + 1;
    const info = getPointsPeriodInfo();

    const embed = new EmbedBuilder()
      .setColor(0xFFD700)
      .setTitle('⭐ Your Chef Points')
      .setThumbnail(interaction.user.displayAvatarURL())
      .addFields(
        { name: 'Points', value: String(pts), inline: true },
        { name: 'Rank', value: rank > 0 ? '#' + rank + ' of ' + sorted.length : 'Unranked', inline: true },
        { name: 'Resets In', value: info.daysLeft + ' day' + (info.daysLeft !== 1 ? 's' : ''), inline: true },
      )
      .setFooter({ text: 'Share more DoorDash links to climb the leaderboard!' })
      .setTimestamp();
  ...
  if (cmd === 'resetpoints') {
    ...
    resetAllPoints();
    return interaction.reply({ embeds: [new EmbedBuilder().setColor(0xF44336).setTitle('🔄 Points Reset').setDescription('All Chef points have been reset to 0. A fresh 30-day period has started!').setTimestamp()] });
  }
...
client.once('ready', async function() {
  ...
  // Check for monthly points reset on startup and every 6 hours
  checkMonthlyReset();
