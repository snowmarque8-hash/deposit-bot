...
function cleanCredentialsJson(raw) {
  if (!raw) return null;
  let cleaned = raw.trim().replace(/\t/g, '').replace(/\r/g, '').replace(/\n/g, '');
...
async function setupGmailAccount(credentialsJson, label) {
  try {
    const cleaned = cleanCredentialsJson(credentialsJson);
    if (!cleaned) throw new Error('No credentials provided');
    const auth = await getGmailAuth(cleaned, label);
