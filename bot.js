...
client.on('messageCreate', async message => {
  ...
  if (giftMatch) {
    ...
    const embed = new EmbedBuilder()
      .setColor(0xFF3008)
      .setTitle('🍔 DoorDash Gift Order Shared!')
      .setURL(url)
      .setDescription('A DoorDash gift order link was shared! Click the link above to view the order details.')
      .addFields(
        { name: '👤 Shared by', value: `<@${message.author.id}>`, inline: true },
        { name: '🔗 Link', value: `[View Order](${url})`, inline: true },
      );

    if (pointsText) embed.addFields({ name: '⭐ Points', value: pointsText });
    embed.setFooter({ text: 'DoorDash gift links require opening in browser to view full details' }).setTimestamp();
