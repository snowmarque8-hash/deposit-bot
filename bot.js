...
client.on('messageCreate', async function(message) {
  ...
  if (drdMatch) {
    ...
    if (fullUrl.includes('group-order') || fullUrl.includes('cart') || fullUrl.includes('drd.sh')) {
      ...
      // Parse what we can from the URL itself as fallback
      let store = 'Unknown';
      let creator = 'Unknown';
      try {
        const urlObj = new URL(fullUrl);
      ...
      const items = (cartData && (cartData.items || (cartData.cart && cartData.cart.items))) || [];
      const subtotal = cartData ? (cartData.subtotal || (cartData.cart && cartData.cart.subtotal) || 0) / 100 : 0;
