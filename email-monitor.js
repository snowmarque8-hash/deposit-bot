    ...
    imap.once('ready', () => {
      imap.openBox('INBOX', false, (err) => {
        ...
        imap.search(['UNSEEN', ['SINCE', since]], (err, results) => {
          ...
          console.log(`📧 [${label}] Found ${results.length} unread email(s)`);
            ...
            const p = new Promise((res) => {
              msg.on('body', (stream) => {
                simpleParser(stream, async (err, parsed) => {
                  ...
                  if (payment) {
                    console.log(`💰 [${label}] Payment detected: ${payment.platform} $${payment.amount}`);
          ...
          fetch.once('end', async () => { await Promise.all(promises); imap.end(); });
